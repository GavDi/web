...
let obj1 = {
    ...
    outCountries: function() {
        let countriesByDima = this.countries.map(function(countriesByDimaArray){
            let state, capital, continent, partOfTheWorld, timeDifference, monetaryUnit, geographicalObjects, literaryWorks, historicalEvents;
            for (let i in countriesByDimaArray) {
                if (typeof countriesByDimaArray[i] === 'string') {
                    if (state === undefined) state = countriesByDimaArray[i];
                    else if (capital === undefined) capital = countriesByDimaArray[i];
                    else if (continent === undefined) continent = countriesByDimaArray[i];
                    else if (partOfTheWorld === undefined) partOfTheWorld = countriesByDimaArray[i];
                    else monetaryUnit = countriesByDimaArray[i];
                }
                else if (typeof countriesByDimaArray[i] === 'number') {
                    timeDifference = countriesByDimaArray[i];
                }
                else if (Array.isArray(countriesByDimaArray[i]) && !Array.isArray(countriesByDimaArray[i][0])) {
                    geographicalObjects = countriesByDimaArray[i];
                    Array.isArray()
                }
                else if (Array.isArray(countriesByDimaArray[i]) && Array.isArray(countriesByDimaArray[i][0])) {
                    let temp_arr = [];
                    for (let j in countriesByDimaArray[i]) {
                        let str = '';
                        let qq = 0;
                        for (let k = 0; k < countriesByDimaArray[i][j].length; k++) {
                            if(countriesByDimaArray[i][j][k] != undefined){
                                str += " ";
                                str += countriesByDimaArray[i][j][k];
                                qq = 1;
                            }
                            else{
                                qq = 0;
                    ...
                    literaryWorks = temp_arr;
                }
                else if (typeof countriesByDimaArray[i] === "object") {
                    historicalEvents = countriesByDimaArray[i];
                }
            }
            let temp = [state, capital, continent, partOfTheWorld, timeDifference, monetaryUnit, geographicalObjects, literaryWorks, historicalEvents];
            return temp;
        });

        countriesByDima.forEach(function(countriesByDimaArray){
            let string = '<table border="0" style="text-align: center">' +
            "<tr><td><b>" + about[0] + "</b></td><td>" + countriesByDimaArray[0] + "</td></tr>" +
            "<tr><td><b>" + about[1] + "</b></td><td>" + countriesByDimaArray[1] + "</td></tr>" +
            "<tr><td><b>" + about[2] + "</b></td><td>" + countriesByDimaArray[2] + "</td></tr>" +
            "<tr><td><b>" + about[3] + "</b></td><td>" + countriesByDimaArray[3] + "</td></tr>" +
            "<tr><td><b>" + about[4] + "</b></td><td>" + countriesByDimaArray[4] + "</td></tr>" +
            "<tr><td><b>" + about[5] + "</b></td><td>" + countriesByDimaArray[5] + "</td></tr>" +
            "<tr><td><b>" + about[6] + "</b></td><td>";
            for (let i in countriesByDimaArray[6]) 
                string += countriesByDimaArray[6][i] + "<br>";
            string += "</td></tr><tr><td><b>" + about[7] + "</b></td><td>";
            for (let i in countriesByDimaArray[7]) 
                string += countriesByDimaArray[7][i] + "<br>";
            string += "</td></tr><tr><td><b>" + about[8];
            string += "<td>";
            for (let k in countriesByDimaArray[8]) 
                string += countriesByDimaArray[8][k] + " " + k + ";<br> ";
            string +="</td>";
    ...
    function(data){
        for (let i in this.countries){
            for (let j = 0; j < countries[i].length; j++){
                if (Array.isArray(this.countries[i][j]) && Array.isArray(this.countries[i][j][0])) {
                    var massk = [];
                    for (let k = 0; k < this.countries[i][j].length; k++){
                        if(this.countries[i][j][k][2] >= data){
                            massk[k] = this.countries[i][j][k];
                        }
                        else{
                            massk[k] = [undefined,undefined];
                        }
                    }
                    this.countries[i][j] = massk;
                }
            }
        }
    }
...
obj2.changeCountries.call(obj1, data);
obj2.changeCountries.apply(obj1, [data]);*/